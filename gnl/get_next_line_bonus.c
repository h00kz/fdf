...
/*   Updated: 2022/10/13 14:57:56 by jlarrieu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_get_aft_nl(char *s_save)
{
	...
	while (s_save[i] != '\n' && s_save[i])
		i++;
	i++;
	tmp = ft_calloc((ft_strlen_g(s_save) - i) + 1, sizeof(char));
	if (!tmp)
		return (NULL);
	while (s_save[i + j])
	{
...
char	*get_next_line(int fd)
{
	char		*line;
	char		*f_line;
	static char	*s_save[1024] = {NULL};

	line = NULL;
	if (fd < 0 && BUFFER_SIZE <= 0)
		return (NULL);
	f_line = ft_get_line(fd, s_save[fd]);
	if (!f_line)
		return (NULL);
	if (!(*f_line))
	{
	...
	s_save[fd] = ft_get_aft_nl(f_line);
	return (line);
}
